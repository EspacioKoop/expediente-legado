#!/usr/bin/env python3
"""Materializa el lote Styloo de #223 en un checkout real con Git LFS.
...
pasa por ``git add`` y comprueba que el índice contiene punteros LFS reales.
...
try:
    from preparar_school_classrooms_styloo import (
        MANIFIESTO_DEFECTO,
        StylooImportError,
        cargar_manifiesto,
        seleccionar_assets,
    ...
    from preparar_school_classrooms_styloo import (  # type: ignore  # noqa: E402
        MANIFIESTO_DEFECTO,
        StylooImportError,
        cargar_manifiesto,
        seleccionar_assets,
...
DESTINO_REL = Path("godot/assets/modelos/styloo_school")
...
def entrada_procedencia(asset: dict, manifiesto: dict) -> dict:
    ...
    return {
        ...
        "sha256": asset["sha256"],
        "archivo_origen": asset["miembro_zip"],
...
def validar_rutas_limpias(repo: Path, rutas: list[Path]) -> None:
    ...
    if salida.strip():
        raise MaterializacionError(
            "Hay cambios locales o staged en rutas que #223 necesita modificar; "
...
def materializar(
    ...
    try:
        ...
        for asset, relativo in zip(seleccion, destinos):
            ...
            absoluto.write_bytes(extraidos[asset["id"]])
...
def ejecutar(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("zip", type=Path, help="ZIP auditado de Styloo")
    ...
    parser.add_argument(
        ...
        action="append",
        dest="lotes",
        help="Lote a materializar; repetible. Por defecto: administrativo.",
    )
    parser.add_argument(
        "--aceptar-reempaquetado",
        action="store_true",
        help="Valida por hash de GLB si el contenedor ZIP fue reempaquetado.",
    )
    parser.add_argument(
        "--aplicar",
        action="store_true",
    ...
    try:
        ...
        seleccion = seleccionar_assets(manifiesto, args.lotes or ["administrativo"])
        ...
        if not args.aplicar:
            for asset, destino in zip(seleccion, destinos):
                print(f"{asset['id']:18s} -> {destino.as_posix()} sha256={asset['sha256']}")
            print(
        ...
        staged, añadidas = materializar(repo, manifiesto, seleccion, extraidos)
        ...
        return 0
    except (OSError, json.JSONDecodeError, StylooImportError, MaterializacionError) as exc:
