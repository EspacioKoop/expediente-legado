...
DIA_ONBOARDING = ROOT / "godot" / "guion" / "dia_onboarding_app.gd"
...
class Caidas784Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dia_onboarding = DIA_ONBOARDING.read_text(encoding="utf-8")
    ...
    def test_caida_profunda_reutiliza_el_contrato_de_situar(self):
        self.assertIn("const UMBRAL_RESCATE_CAIDA := -8.0", self.dia_onboarding)
        self.assertIn("_caminante.position.y >= UMBRAL_RESCATE_CAIDA", self.dia_onboarding)
    ...
    def test_salida_disparada_en_fisica_sale_del_callback_antes_de_transicionar(self):
        ...
        self.assertNotIn('call_deferred("_al_pisar_salida"', self.dia_onboarding)
        self.assertIn("super._al_pisar_salida(cuerpo, salida)", self.dia_onboarding)

    def test_cadena_historica_permanece_intacta(self):
        self.assertIn('extends "res://guion/dia_onboarding_app.gd"', self.dia_calle)
        self.assertIn('extends "res://guion/dia_gato_app.gd"', self.dia_onboarding)
        self.assertIn('path="res://guion/dia_clima_app.gd" id="1"', self.escena_dia)
        self.assertNotIn("dia_caidas_app.gd", self.dia_calle)
