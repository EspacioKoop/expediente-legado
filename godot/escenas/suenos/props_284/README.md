...
- `torre_castillo_psx.obj`: torre autónoma recentrada, derivada de la misma portada original; conserva cuerpo octogonal, cubierta apuntada y saetera sin arrastrar muro/arco.
- `torre_castillo.tscn`: módulo reutilizable de torre para campanario, torreones y composiciones verticales.
...
- `torre_capilla_castillo.tscn`: nave de capilla formada por arcadas propias y un campanario autónomo; una segunda torre invertida/elevada y dos escaleras rompen la lectura vertical sin crear una segunda navegación.
...
- `SuenoCastilloUmbrales3D`: controlador visual que reutiliza un umbral ya presente en cada composición para insinuar la siguiente ala con miniaturas de piezas propias; no crea teletransporte, colisión ni una segunda planta caminable.
...
El castillo dispone ahora de cuatro composiciones sobre el mismo lenguaje arquitectónico: patio, scriptorium, torre/capilla y claustro reflejado. `SuenoCastillo` selecciona la lectura y una mutación secundaria de forma determinista; `SuenoCastillo3D` puede desfazar, contraer/estirar o girar la composición sin tocar la física ANULAR. Además, piezas concretas declaradas como `castillo_anomalia` responden con microdesplazamientos a las tres campanadas procedurales, mientras suelo, silueta principal y navegación permanecen inmóviles. Un segundo contrato, `castillo_umbral`, conecta visualmente las composiciones: patio insinúa scriptorium, scriptorium claustro, claustro torre/capilla y torre/capilla patio mediante ecos de la misma arquitectura propia. La zona de proximidad del códice conserva ahora un `evento` semántico: al leerla, las anomalías se tensan y las luces de esos umbrales se refuerzan durante la visita, sin persistir nada ni alterar la interacción común. Así una misma ala puede reaparecer con pequeñas contradicciones espaciales y pertenecer a un castillo continuo, en vez de funcionar como un preset aislado. Tras #587 y #755, esta vertical cubre la necesidad que motivó #296; PSX Going Medieval y Fantasy Props MegaKit quedan como **reserva opcional**, no como dependencia ni deuda de importación.
