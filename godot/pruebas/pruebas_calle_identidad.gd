...
func _probar_tienda(dia, calle: Node3D) -> void:
	...
	for entrada in catalogo:
		presupuesto += int(entrada["precio"])
	dia.jornada["dinero"] = presupuesto
	...
	if hay_stock:
		...
		_comprobar(
			int(dia.jornada["dinero"]) == presupuesto - int(primera["precio"]),
			"cobra el precio"
		)
		...
		_comprobar(int(dia.jornada["dinero"]) == presupuesto - gastado, "no cobra dos veces")
		_comprobar(
		...
		_comprobar(int(dia.jornada["dinero"]) == presupuesto, "sin existencias no cobra")
		_comprobar(
