...
func _ejecutar() -> void:
	...
	var evento_escaparate := SenalDatos.crear_evento(
		"calle", "test", "anon-a", "calle_escaparate", "cuidado_con", ["trampa"], AHORA, [], -1, "sig-1"
	)
	...
	_comprobar("SIGA no admite anchors", not invalida["ok"] and invalida["status"] == "unknown_anchor")
	...
	_comprobar("salir desmonta señales", host._mundo.get_node_or_null("SenalesMultiplayerCalle") == null)
