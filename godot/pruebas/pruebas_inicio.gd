...
func _probar() -> void:
	...
	var marca := inicio.get_node_or_null("CentroInicio/MarcoInicio/BarraTituloInicio/MarcaInicio")
	_comprobar(marca is TextureRect and marca.texture != null, "cabecera usa marca visual propia")
	var marco := inicio.get_node_or_null("CentroInicio/MarcoInicio")
	_comprobar(marco != null and marco.saliente, "marco principal conserva relieve saliente")
	var estado := inicio.get_node_or_null("CentroInicio/MarcoInicio/EstadoInicio")
	_comprobar(estado != null and not estado.saliente, "estado queda hundido respecto al marco")
	_comprobar(inicio._continuar.custom_minimum_size.y >= 36.0, "acciones tienen presencia visual suficiente")
