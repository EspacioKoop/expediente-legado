...
func _probar() -> void:
	...
	_comprobar(marco != null and marco.get("saliente") == true, "marco principal conserva relieve saliente")
	var estado := inicio.find_child("EstadoInicio", true, false)
	_comprobar(estado != null and estado.get("saliente") == false, "estado queda hundido respecto al marco")
