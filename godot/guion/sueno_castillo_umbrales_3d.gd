...
static func montar(arquitectura: Node3D, variante: String) -> Node3D:
	...
	for candidato in arquitectura.find_children("*", "Node3D", true, false):
		...
		eco.position = Vector3(0.0, 0.0, -3.1)
...
static func _montar_motivo(eco: Node3D, destino: String) -> void:
	match destino:
		"torre_capilla":
			_instanciar(eco, ESCENA_TORRE, "TorreLejana", Vector3(0.0, 0.0, -1.8), 0.42)
		"claustro_reflejado":
			_instanciar(eco, ESCENA_ARCADA, "ArcadaEcoA", Vector3(-1.15, 0.0, -1.35), 0.42)
			_instanciar(eco, ESCENA_ARCADA, "ArcadaEcoB", Vector3(1.15, 0.0, -1.35), 0.42)
		"patio":
			_instanciar(eco, ESCENA_PORTADA, "PortadaLejana", Vector3(0.0, 0.0, -1.8), 0.28)
		_:
			_instanciar(eco, ESCENA_ESCALERA, "EscaleraArchivoLejana", Vector3(0.0, 0.0, -1.4), 0.34)
			var estandarte := _instanciar(
				eco,
				ESCENA_ESTANTARTE,
				"EstandarteArchivoLejano",
				Vector3(0.9, 1.15, -1.55),
				0.48,
			)
...
static func _montar_luz(eco: Node3D) -> void:
	...
	luz.name = "LuzUmbral"
	luz.position = Vector3(0.0, 1.7, -1.0)
	luz.light_color = Color(0.46, 0.37, 0.28, 1.0)
	luz.light_energy = 0.24
	luz.omni_range = 4.2
