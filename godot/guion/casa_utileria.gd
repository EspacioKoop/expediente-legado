...
static func _montar_sofa(raiz: Node3D, pos: Vector3, giro_y: float) -> void:
	...
	_agregar_caja(sofa, Vector3(0, 0.78, 0.30), Vector3(1.80, 0.88, 0.18), tela_oscura, "moqueta")
	_agregar_caja(sofa, Vector3(-0.87, 0.52, 0), Vector3(0.16, 0.52, 0.72), tela_oscura, "moqueta")
	_agregar_caja(sofa, Vector3(0.87, 0.52, 0), Vector3(0.16, 0.52, 0.72), tela_oscura, "moqueta")
...
static func _montar_cocina(raiz: Node3D, pos: Vector3) -> void:
	...
	_agregar_caja(cocina, Vector3(0, 0.45, 0), Vector3(0.56, 0.90, 1.80), mueble, "melamina")
	_agregar_caja(cocina, Vector3(-0.02, 0.93, 0), Vector3(0.64, 0.08, 1.92), encimera, "melamina")

	var fregadero := Node3D.new()
	fregadero.name = "FregaderoCasa"
	fregadero.position = Vector3(-0.03, 0.99, 0.30)
	cocina.add_child(fregadero)
	_agregar_caja(fregadero, Vector3.ZERO, Vector3(0.44, 0.035, 0.58), metal, "metal_pintado")
	_agregar_cilindro(fregadero, Vector3(0.12, 0.20, 0.12), 0.025, 0.36, metal, "metal_pintado")
	_agregar_caja(
		fregadero, Vector3(0.08, 0.36, 0.12), Vector3(0.22, 0.04, 0.04), metal, "metal_pintado"
	)
	...
	_agregar_caja(
		nevera, Vector3(-0.37, 1.16, -0.23), Vector3(0.035, 0.52, 0.07), metal, "metal_pintado"
	)
	_agregar_caja(
		nevera, Vector3(-0.37, 0.55, -0.23), Vector3(0.035, 0.34, 0.07), metal, "metal_pintado"
	)
...
static func _montar_ventana(raiz: Node3D, pos: Vector3) -> void:
	...
	_agregar_caja(ventana, Vector3(0, 0.58, 0), Vector3(1.94, 0.10, 0.08), marco, "melamina")
	_agregar_caja(ventana, Vector3(0, -0.58, 0), Vector3(1.94, 0.10, 0.08), marco, "melamina")
	_agregar_caja(ventana, Vector3(-0.92, 0, 0), Vector3(0.10, 1.18, 0.08), marco, "melamina")
	_agregar_caja(ventana, Vector3(0.92, 0, 0), Vector3(0.10, 1.18, 0.08), marco, "melamina")
	_agregar_caja(ventana, Vector3(0, 0, 0), Vector3(0.07, 1.08, 0.075), marco, "melamina")
...
static func _montar_estanteria_compras(raiz: Node3D, pos: Vector3) -> void:
	...
	_agregar_caja(estanteria, Vector3(0.48, 0.82, 0), Vector3(0.10, 1.64, 0.34), madera, "melamina")
	for y in [0.08, 0.58, 1.08, 1.58]:
		_agregar_caja(estanteria, Vector3(0, y, 0), Vector3(1.02, 0.09, 0.36), madera, "melamina")
...
static func _montar_mesita(raiz: Node3D, pos: Vector3) -> void:
	...
	_agregar_caja(
		mesa, Vector3(0, 0.55, 0), Vector3(0.82, 0.12, 0.62), Color(0.32, 0.23, 0.17), "melamina"
	)
	for x in [-0.31, 0.31]:
		for z in [-0.21, 0.21]:
			_agregar_caja(
...
static func _montar_lampara_pie(raiz: Node3D, pos: Vector3) -> void:
	...
	_agregar_cilindro(
		lampara, Vector3(0, 0.05, 0), 0.28, 0.10, Color(0.18, 0.17, 0.16), "metal_pintado"
	)
	_agregar_cilindro(
		lampara, Vector3(0, 0.82, 0), 0.045, 1.55, Color(0.26, 0.24, 0.22), "metal_pintado"
	)
