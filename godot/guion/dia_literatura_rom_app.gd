...
func _process(_delta: float) -> void:
	...
	if typeof(registro_variante) != TYPE_DICTIONARY:
		return
	var registro: Dictionary = registro_variante
	var desbloqueadas := LiteraturaRoms.desbloqueadas(registro)
	for id_rom in desbloqueadas:
		consola.desbloquear_rom(String(id_rom))

	if not desbloqueadas.has(Sueno98Vigilia.ID_ROM):
		return
	if is_instance_valid(_observador):
		return
	...
	_observador.configurar(gestor, dia.jornada, consola)
